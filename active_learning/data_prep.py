...
def get_data(random_state: int = 42, dataset: str = "ALDH1"):
    ...
    for smi in tqdm(actives):
        try:
            if (
                Chem.MolFromSmiles(smi_to_scaff(smi, includeChirality=False))
                is not None
            ):
...
def split_data(
    ...
    # write to csv
    df_screen.to_csv(
        os.path.join(ROOT_DIR, f"data/{dataset}/original/screen.csv"), index=False
    )
    df_test.to_csv(
        os.path.join(ROOT_DIR, f"data/{dataset}/original/test.csv"), index=False
    )
...
class MasterDataset:
    ...
    def process(self, df: pd.DataFrame) -> None:
        ...
        graphs = [
            smiles_to_graph(smi, y=y.type(torch.LongTensor))
...
class MasterDatasetPath:
    """Dataset that holds all data in an indexable way from a file instead of dataset name"""
...
def smi_to_scaff(smiles: str, includeChirality: bool = False):
    return MurckoScaffold.MurckoScaffoldSmiles(
        mol=Chem.MolFromSmiles(smiles), includeChirality=includeChirality
    )
...
def similarity_vectors(df_screen, df_test, root: str = "data", dataset: str = "ALDH1"):
    ...
    S = get_tanimoto_matrix(
        df_test["smiles"].tolist(),
        verbose=True,
        scaffolds=False,
        zero_diag=True,
        as_vector=True,
...
if __name__ == "__main__":
    for dataset in ["ALDH1", "PKM2", "VDR"]:
        ...
        df_screen, df_test = split_data(
            df, screen_size=100000, test_size=20000, dataset=dataset
        )
        ...
        df_screen = pd.read_csv(
            os.path.join(ROOT_DIR, f"data/{dataset}/original/screen.csv")
        )
        df_test = pd.read_csv(
            os.path.join(ROOT_DIR, f"data/{dataset}/original/test.csv")
        )
