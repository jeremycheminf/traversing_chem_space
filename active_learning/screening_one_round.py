...
from active_learning.data_prep import MasterDatasetPath
...
def active_learning_one_round(
    acquisition_method: str = "exploration",
    ...
    retrain: bool = True,
    anchored: bool = True,
    path_file_prior: str = None,
    path_file_select: str = None,
) -> pd.DataFrame:
    """
    ...
    ds_train = MasterDatasetPath(
        "train", representation=representation, path=path_file_prior, select=False
    )
    ds_select = MasterDatasetPath(
        "select", representation=representation, path=path_file_select, select=True
    )
    ...
    train_loader_balanced = to_torch_dataloader(
        x_train,
        y_train,
    ...
    screen_loader = to_torch_dataloader(
        x_screen,
        y_screen,
        batch_size=INFERENCE_BATCH_SIZE,
        num_workers=NUM_WORKERS,
    ...
    picks = ACQ.acquire(
        screen_logits_N_K_C, smiles_screen, hits=smiles_train, n=batch_size
    )
    # handler.add(picks)
